...
"use strict";angular.module("sipayApp",["ngAnimate","ngMessages","ngRoute","ngSanitize","ngTouch","ngDialog","pascalprecht.translate"]).config(["$translateProvider","SUPPORTED_LANGUAGES","DEFAULT_LANGUAGE","TRANSLATIONS",function(a,b,c,d){for(var e="undefined"!=typeof LOCALE?LOCALE:c,f=0;f<b.length;f++){var g=b[f];a.translations(g,d[g])}a.preferredLanguage(e)}]).config(["$routeProvider",function(a){a.when("/main/:lang/:error?",{templateUrl:"views/main.html",controller:"MainCtrl",controllerAs:"main"}).when("/about",{templateUrl:"views/about.html",controller:"AboutCtrl",controllerAs:"about"}).when("/",{templateUrl:"views/payment.html",controller:"PaymentCtrl",controllerAs:"payment"}).otherwise({redirectTo:"/"})}]),angular.module("sipayApp").controller("MainCtrl",["$sce","$routeParams","$translate","ngDialog","$interval","IFRAME_TIMEOUT_INTERVAL",function(a,b,c,d,e,f){this.establishment=data.establishment,this.showCart=data.cart.total>0,this.iframeSource=a.trustAsResourceUrl(sipayData.sipayIframe),this.goBack=function(){window.opener?window.close():window.history.back()};var g=new Date(sipayData.sipayStart);g=g.setSeconds(g.getSeconds()+f);var h=Math.abs(g-new Date);e(function(){window.location.reload()},h,1,!0),c.use(b.lang),!b.error||"sipayError"!==b.error&&"sipayErrorCard"!==b.error||d.open({template:"<p>"+c.instant("trans.step2."+b.error)+"</p>",plain:!0})}]),angular.module("sipayApp").controller("AboutCtrl",function(){this.awesomeThings=["HTML5 Boilerplate","AngularJS","Karma"]}),angular.module("sipayApp").controller("PaymentCtrl",function(){}),angular.module("sipayApp").directive("cart",function(){return{templateUrl:"views/cart.html",restrict:"AE",link:function(a,b,c){a.cart=data.cart,a.showCart=!1,a.toggleCart=function(b){b.preventDefault(),a.showCart=!a.showCart}}}}),angular.module("sipayApp").directive("header",function(){return{templateUrl:"views/header.html",restrict:"AE",link:function(a,b,c){}}}),angular.module("sipayApp").directive("paymentForm",["SUBMIT_URL",function(a){return{templateUrl:"views/paymentForm.html",restrict:"AE",link:function(b,c,d){b.paymentData={},b.paymentData.creditCardData={},b.submit=function(){document.getElementById("loader").style.display="block",document.getElementById("paymentForm").action=a,document.getElementById("paymentForm").submit()};for(var e=11,f=[],g=[],h=null,i=new Date,j=1;e>j;j++)h=i.getFullYear().toString().substr(2,2),f.push({id:j,value:h,name:h}),i.setFullYear(i.getFullYear()+1);for(b.years=f,j=1;13>j;j++)h=9>=j?"0"+j.toString():j.toString(),g.push({id:j,value:h,name:h});b.months=g,b.creditCards=[{id:"Mastercard",name:"Eurocard/MasterCard"},{id:"American_Express",name:"American Express"},{id:"Visa",name:"Visa"}];var k={Visa:/^4[0-9]{12}(?:[0-9]{3})?$/,Mastercard:/^5[1-5][0-9]{14}$/,American_Express:/^3[47][0-9]{13}$/,Carte_Bleu:/^[0-9]*$/,Diners:/^3(?:0[0-5]|[68][0-9])[0-9]{11}$/,JCB:/^(?:2131|1800|35\\d{3})\\d{11}$/};b.ccPattern=k[b.creditCards[0].id],b.changePattern=function(a){b.ccPattern=k[a.id],b.paymentForm.numbercard.$setViewValue(b.paymentForm.numbercard.$viewValue)},b.paymentData.creditCardData.cckind=b.creditCards[0]}}}]),angular.module("sipayApp").constant("SUBMIT_URL","http://localhost:9091/WitBooker/payment/sipayResult?ticker=hoteldemo.com.v6").constant("SUPPORTED_LANGUAGES",["en","es"]).constant("DEFAULT_LANGUAGE","es").constant("IFRAME_TIMEOUT_INTERVAL",3e5).constant("TRANSLATIONS",{en:{"trans.step2.ccCardText":"Your credit card acts as a <b>payment guarantee</b> to cover cancelation or no-show costs acording to the service terms and conditions.","trans.step2.cCHolder":"Card holder","trans.step2.cCNumber":"Card number","trans.step2.validUntil":"Valid until","trans.step2.month":"month","trans.step2.year":"year"},ca:{"trans.step2.ccCardText":"La seva targeta de crèdit serveix com a <b>garantia</b> de <b>pagament</b> i només s'emprarà en cas de cancel·lació d'acord amb les condicions de servei.","trans.step2.cCHolder":"Titular de la tarja","trans.step2.cCNumber":"Número tarja","trans.step2.validUntil":"Vàlida fins","trans.step2.month":"mes","trans.step2.year":"any"},es:{"trans.step2.ccCardText":"Su tarjeta de crédito sirve como <b>garantía de pago</b> y se empleará de acuerdo con las condiciones de servicio.","trans.step2.cCHolder":"Titular de la tarjeta","trans.step2.cCNumber":"Número tarjeta","trans.step2.validUntil":"Válida hasta","trans.step2.month":"mes","trans.step2.year":"año","trans.step2.sipayError":"Ha ocurrido un error procesando el pago, por favor intente nuevamente.","trans.step2.sipayErrorCard":"Ha ocurrido un error procesando el pago, la tarjeta no pudo ser procesada.","trans.step2.goBack":"Cancelar y volver"},pt:{"trans.step2.ccCardText":"O seu cartão de crédito é necessário porque irá garantir a sua reserva. Não será cobrado nenhum depósito.","trans.step2.cCHolder":"Titular do cartão","trans.step2.cCNumber":"Número do cartão","trans.step2.validUntil":"Válido até","trans.step2.month":"mês","trans.step2.year":"ano"},it:{"trans.step2.ccCardText":"La tua carta di credito funge da <b>garanzia di pagamento</b> a copertura cancellazione o no-show costi acording i termini e le condizioni di servizio.","trans.step2.cCHolder":"Titolare della carta di credito","trans.step2.cCNumber":"Numero carta di credito","trans.step2.validUntil":"Validità","trans.step2.month":"mese","trans.step2.year":"anno"},de:{"trans.step2.ccCardText":"Ihre Kreditkarte dient als <b>Zahlungsgarantie</b> für Stornierung oder No-Show-Kosten mit Monitoren nach den Service-Bedingungen und Konditionen zu decken.","trans.step2.cCHolder":"Karteninhaber","trans.step2.cCNumber":"Kartennummer","trans.step2.validUntil":"Gültig bis","trans.step2.month":"Monat","trans.step2.year":"Jahr"},ru:{"trans.step2.ccCardText":"Ваша кредитная карта выступает в качестве <b>гарантии оплаты</b> для покрытия отмены или не показывать расходы acording на службу условиях.","trans.step2.cCHolder":"Держатель карты","trans.step2.cCNumber":"Номер карты","trans.step2.validUntil":"Действительно до","trans.step2.month":"Месяц","trans.step2.year":"Год"},fr:{"trans.step2.ccCardText":"Votre carte de crédit tient lieu de <b>garantie de paiement </b> et <b>elle ne sera utilisée qu’en cas d’annulation </b>conformément aux conditions de service.","trans.step2.cCHolder":"Titulaire de la carte","trans.step2.cCNumber":"Numéro de la carte","trans.step2.validUntil":"Valable jusqu’à","trans.step2.month":"mois","trans.step2.year":"année"},nl:{"trans.step2.ccCardText":"Uw creditcard is nodig om uw reservering te garanderen. U hoeft geen aanbetaling te doen.","trans.step2.cCHolder":"Creditkaarthouder","trans.step2.cCNumber":"Kaartnummer","trans.step2.validUntil":"Geldig tot","trans.step2.month":"maand","trans.step2.year":"jaar"},ar:{"trans.step2.ccCardText":"Your credit card acts as a <b>payment guarantee</b> to cover cancelation or no-show costs acording to the service terms and conditions.","trans.step2.cCHolder":"Card holder","trans.step2.cCNumber":"Card number","trans.step2.validUntil":"Valid until","trans.step2.month":"month","trans.step2.year":"year"},pl:{"trans.step2.ccCardText":"Your credit card acts as a <b>payment guarantee</b> to cover cancelation or no-show costs acording to the service terms and conditions.","trans.step2.cCHolder":"Card holder","trans.step2.cCNumber":"Card number","trans.step2.validUntil":"Valid until","trans.step2.month":"month","trans.step2.year":"year"},zh:{"trans.step2.ccCardText":"Your credit card acts as a <b>payment guarantee</b> to cover cancelation or no-show costs acording to the service terms and conditions.","trans.step2.cCHolder":"Card holder","trans.step2.cCNumber":"Card number","trans.step2.validUntil":"Valid until","trans.step2.month":"month","trans.step2.year":"year"}}),angular.module("sipayApp").run(["$templateCache",function(a){a.put("views/about.html","<p>This is the about view.</p>"),a.put("views/cart.html",'<div> <div class="cartContainer" content="xospartaweb"> <div class="transactionDetailsContainer" ng-class="{cart:showCart}"> <a href="#transactionCartDetails" class="transactionDetails scTrack:hermes-cartExpanded" aria-controls="transctionCartDetails" aria-label="Shopping cart" role="button" ng-click="toggleCart($event)"> <span class="totalWrapper clearfix" id="totalWrapper"> <span aria-label="Your Cart" id="yourCart" class="accessAid"></span> <span id="transactionCart" class="hasItems"> <span class="cartIcon"></span> <span> <format-currency code="checkoutCart.purchase.amounts.total.currency_code" amount="checkoutCart.purchase.amounts.total.amount">{{cart.total | number:2}} {{cart.currency.symbol}}</format-currency> </span> <span class="arrow"> </span> </span> </span> </a> <!--TODO: TO SHOW   /* overflow: hidden; */ AND   /* visibility: hidden; */: on .transctionCartDetails CLASS --> <div class="transctionCartDetails" aria-hidden="true"> <div class="details"> <h3 class="merchantName">{{main.establishment.name}}</h3> <ul class="detail-items"> <li ng-repeat="item in cart.items" class="items"> <span class="itemName">{{item.name}}</span> <span class="itemPrice"> <format-currency code="item.amount.currency_code" amount="item.amount.amount"> {{item.total | number:2}} </format-currency> <span class="currencyCode">{{cart.currency.symbol}}</span> </span> </li> <li class="subTotal"> <span class="itemName">Total</span> <span class="itemPrice"> <format-currency code="checkoutCart.purchase.amounts.total.currency_code" amount="checkoutCart.purchase.amounts.total.amount"> {{cart.total | number:2}} </format-currency> <span class="currencyCode">{{cart.currency.symbol}}</span> </span> </li> </ul> <!--<a href="#" id="closeCart" role="button" class="actions" ng-click="closeCart($event)">--> <!--<span class="accessAid">Cerrar</span>--> <!--</a>--> </div> <div class="pointer"></div> </div> </div> </div> </div>'),a.put("views/header.html",'<div id="header" class="merchantHeader clearfix"> <div class="headerWrapper"> <span class="merchantName ng-binding"> {{main.establishment.name}} </span> </div> </div>'),a.put("views/loader.html",'<!DOCTYPE html> <html lang="en"> <head><title>Sipay</title> <style>.mask {\n      z-index: 20001;\n      position: absolute;\n      top: 0;\n      left: 0;\n      background-color: black;\n      opacity: 0.54;\n      filter: alpha(opacity=54);\n    }\n\n    .ppmodal {\n      display: block;\n      z-index: 20003;\n      -webkit-box-sizing: border-box;\n      -moz-box-sizing: border-box;\n      -ms-box-sizing: border-box;\n      box-sizing: border-box;\n      width: 100%;\n      height: 100%;\n      top: 0;\n      left: 0;\n      overflow: hidden;\n      background-color: #ffffff;\n      -moz-background-clip: padding-box;\n      -webkit-background-clip: padding-box;\n      background-clip: padding-box;\n      padding: 2em;\n      font-family: "helvetica heue", arial, helvetica, sans-serif;\n      position: fixed;\n    }\n\n    .minibrowser .ppmodal {\n      display: block;\n    }\n\n    .loading .mask {\n      height: 30px;\n      width: 30px;\n      position: absolute;\n      left: 48%;\n      top: 50%;\n      margin: -15px auto auto -15px;\n      opacity: 1;\n      filter: alpha(opacity=100);\n      background-color: rgba(255, 255, 255, 0.701961);\n    }\n\n    .loading .mask {\n      height: 52px;\n      width: 52px;\n      border: none;\n      background: url(data:image/gif;base64,) no-repeat center center;\n    }\n\n    .ppmodal.loading {\n      min-height: 160px;\n    }</style> </head> <body> <div> <div class="ppmodal loading"> <div class="mask"></div> </div> </div> </body> </html>'),
a.put("views/main.html",'<div> <div class="outerWrapper ES es minibrowser" id="outerWrapper"> <div header></div> <div id="wrapper"> <div id="main"> <div token="token"> <div class="row-fluid paypalHeaderWrapper" content="xospartaweb"> <div class="fullwidth span14 clearfix paypalHeader"> <div id="paypalLogo" class="logo"></div> <div ng-if="showCart" cart></div> </div> </div> </div> <div id="contents"> <div token="token"> <section class="login" id="login" content="xospartaweb"> <div class="row-fluid"> <div class="span14 trayInner"> <div> <section id="login" content="xospartaweb"> <div message="errorMessage"></div> <div class="pr row-fluid" ng-class="{hasError: errorMessage}"> <div class="cardMessageSmall"> <div class="textTopPaymentMethod" translate>trans.step2.ccCardText</div> </div> </div> <iframe ng-src="{{main.iframeSource}}" frameborder="0" style="width: 100%;height: 420px;overflow: hidden" scrolling="no" target="_parent"> </iframe> </section> </div> </div> <div class="cardMessageLarge span10"> <div class="props login"> <div class="prop-img"></div> <h2 class="vprop-header ng-binding">Más seguro. Más rápido. Más fácil.</h2> <p> <div class="textTopPaymentMethod" translate>trans.step2.ccCardText</div> </p> </div> </div> </div> </section> </div> </div> </div> </div> </div> <div token="token"> <footer content="xospartaweb"> <div class="footer clearfix commonFooter" role="contentinfo"> <p id="defaultCancelLink" class="cancelUrl"> <a ng-click="main.goBack()" role="button" id="cancelLink" translate> trans.step2.goBack </a> </p> <div class="footerWrapper"> <ul class="footerLinks"> <li class="first" id="footer_legal"> <a href="http://www.sipay.es/index.php/aviso-legal" class="popup" target="_blank">Acuerdos legales</a> </li> <li class="first" id="footer_legal"> <a href="http://www.sipay.es/index.php/politica-de-privacidad" class="popup" target="_blank">Privacidad</a> </li> </ul> <p class="copyright ng-binding"> © 1999 - 2015<span class="secureIcon"></span></p> </div> </div> </footer> </div> <div token="token" class="ng-isolate-scope"> </div> </div> '),a.put("views/payment.html",'<div id="contents"> <div token="token"> <section class="login" id="login" content="xospartaweb"> <div class="row-fluid"> <div class="span14 trayInner"> <div> <section id="login" content="xospartaweb"> <div message="errorMessage"></div> <div class="pr row-fluid" ng-class="{hasError: errorMessage}"> <div title-txt="Pagar con PayPal"> <h1 class="alpha" translate>trans.content.title</h1> </div> </div> <div payment-form></div> </section> </div> </div> <div class="span10"> <div class="props login"> <div class="prop-img"></div> <h2 class="vprop-header ng-binding">Más seguro. Más rápido. Más fácil.</h2> <p> Bienvenido al nuevo proceso de pago de PayPal. Así es como deberían ser todos los procesos de pago: <br>más rápidos, más seguros y más cómodos. </p> </div> </div> </div> </section> </div> </div>'),a.put("views/paymentForm.html",'<div id="loader" style="display: none"> <div class="ppmodal loading"> <div class="mask"></div> </div> </div> <div class="textTopPaymentMethod" translate>trans.step2.ccCardText</div><br> <form id="paymentForm" name="paymentForm" class="proceed" novalidate autocomplete="off" method="post" target="_parent" ng-submit="paymentForm.$valid && submit()"> <div class="inputField emailField confidential"> <input type="text" name="ccholder" ng-class="{ \'input-error\' : paymentForm.submitted && paymentForm.ccholder.$error.required }" placeholder="* {{\'trans.step2.cCHolder\'|translate}}" ng-model="paymentData.creditCardData.ccholder" required> </div> <div class="inputField confidential"> <select name="cckind" class="ccselect" ng-options="c.name for c in creditCards track by c.id" ng-model="paymentData.creditCardData.cckind" ng-change="changePattern(paymentData.creditCardData.cckind)"> </select> <div class="creditcards {{paymentData.creditCardData.cckind.id}}"></div> </div> <div class="inputField emailField confidential"> <input type="text" name="numbercard" ng-class="{ \'input-error\' : paymentForm.submitted && (paymentForm.numbercard.$error.required || paymentForm.numbercard.$error.pattern ) }" placeholder="* {{\'trans.step2.cCNumber\'|translate}}" ng-model="paymentData.creditCardData.ccnumber" ng-pattern="ccPattern" required> </div> <span class="validTo"> *<span translate>trans.step2.validUntil</span> </span> <div class="inputField confidential"> <select name="ccvalidtomonth" class="ccExpiration" ng-options="item.value as item.name for item in months track by item.id" ng-class="{ \'input-error\' : paymentForm.submitted && paymentForm.ccvalidtomonth.$error.required }" ng-model="paymentData.creditCardData.ccvalidtomonth" required> <option value="">{{\'trans.step2.month\'|translate}}</option> </select> <select name="ccvalidtoyear" class="ccExpiration" ng-options="item.value as item.name for item in years track by item.id" ng-class="{ \'input-error\' : paymentForm.submitted && paymentForm.ccvalidtoyear.$error.required }" ng-model="paymentData.creditCardData.ccvalidtoyear" required> <option value="">{{\'trans.step2.year\'|translate}}</option> </select> </div> <input type="submit" value="Iniciar sesión en PayPal" ng-click="paymentForm.submitted=true" class="btn full loginBtn"> <input type="hidden" name="resultCode" value="0"> <input type="hidden" name="resultDescription" value="Desc"> <input type="hidden" name="authorizationCode" value="Auth"> <input type="hidden" name="ticketNumber" value="Tick"> <input type="hidden" name="internalNumber" value="Int"> </form>')}]);